...
void PlotDrawer::drawPlotNew(const PlotParameters& parameters)
{
    ...
    pixmap = QPixmap{parameters.area_width, parameters.area_height};
    ...
    const auto x_coefficient = parameters.area_width / delta_x;
    const auto y_coefficient = parameters.area_height / delta_y;
    ...
    for (size_t i = first_point + rarefaction; i <= last_point; i += rarefaction) {
        ...
        if (i - rarefaction == first_point) {
            point1.setX(0.);
            point1.setY(parameters.area_height - (y_coefficient * (data[i - rarefaction].y() - parameters.min_values.y())));
        } else {
        ...
        if (i == last_point) {
            ...
            point2.setY(parameters.area_height - (y_coefficient * (data[i].y() - parameters.min_values.y())));
        } else {
    ...
    if (last_point_painted != last_point) {
        ...
        point1.setY(parameters.area_height - (y_coefficient * (data[last_point - rarefaction].y() - parameters.min_values.y())));
        QPointF point2;
        point2.setX(x_coefficient * (data[last_point].x() - parameters.min_values.x()));
        point2.setY(parameters.area_height - (y_coefficient * (data[last_point].y() - parameters.min_values.y())));
        painter.drawLine(point1, point2);
    }
}
...
void PlotDrawer::drawPreview(int width, int height)
{
    ...
    parameters.area_width = width;
    parameters.area_height = height;
